...
const AddBookScreen = ({ navigation }) => {
  ...
  const addNewPage = () => {
    setForm((prev) => ({
      ...prev,
      pages: [...prev.pages, { pageNumber: prev.pages.length + 1, content: '' }],
    }));
  };
  ...
  const pickImage = async () => {
    try {
      ...
      if (!permissionResult.granted) {
        ...
        return Alert.alert('Quyền bị từ chối', 'Cần quyền truy cập thư viện ảnh để chọn ảnh.');
      }
      ...
      console.log('[DEBUG] Image picker result:', JSON.stringify(result, null, 2));
    ...
    try {
      ...
      if (form.image) {
        ...
        formData.append('image', {
          uri: Platform.OS === 'android' ? form.image : form.image.replace('file://', ''),
          name: `book_image.${fileType}`,
          type: `image/${fileType}`,
        });
      }

      console.log('[DEBUG] Sending FormData:', JSON.stringify(formData._parts, null, 2));
      ...
      console.error('[ERROR] Create book error:', error.message, error.config || {}, error.response?.data || {});
      if (error.message === 'No authentication token found') {
        Alert.alert('Lỗi xác thực', 'Vui lòng đăng nhập lại để tiếp tục.');
      } else if (error.code === 'ERR_NETWORK') {
        Alert.alert('Lỗi mạng', 'Không thể kết nối đến server tại http://172.16.40.25:3000. Vui lòng kiểm tra kết nối mạng hoặc server.');
      } else if (error.response) {
        Alert.alert('Lỗi server', `Server trả về lỗi: ${error.response.status} - ${error.response.data.message || 'Không xác định'}`);
      } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      {form.image ? (
        <Image
          source={{ uri: Platform.OS === 'android' ? form.image : form.image.replace('file://', '') }}
